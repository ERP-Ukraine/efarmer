...
class CheckWhitelistDetail(models.TransientModel):
    ...
    def _compute_status(self):
        for rec_id in self:
            rec_id.is_error = rec_id.is_success = rec_id.is_warning = False

            if rec_id.error_type:
                if rec_id.error_type in ('invalid_bank_acc', 'invalid_vat'):
                    rec_id.is_warning = True

                elif rec_id.error_type == 'positive':
                    rec_id.is_success = True
