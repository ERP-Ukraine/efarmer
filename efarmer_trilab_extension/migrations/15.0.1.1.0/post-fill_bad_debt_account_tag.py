...
where
    ...
    env.ref('l10n_pl.3_vz_kraj_23').invoice_repartition_line_ids.filtered(
        lambda r: env.ref('trilab_jpk_vat.pl_jpk_K_46') in r.tag_ids
    ).write({"tag_ids": [Command.link(env.ref('efarmer_trilab_extension.pl_jpk_K_46_K_47').id)]})
