...
from .jpk_vat7m import JpkVat7MReport
...
class JpkReportV3(models.AbstractModel):
    _name = 'report.trilab_jpk_vat.jpk_vat7m_v3_report'
    _inherit = 'report.trilab_jpk_vat.jpk_vat7m_report'
    _description = 'JPK VAT 7M (3) 1.0E Report'

    grouping_columns = JpkVat7MReport.grouping_columns + [CellDefinition('ksefdowod', 'Typ dowodu faktury')]
    ...
    def _get_query():
        return """SELECT am.id                                    AS gid,
                         ...
                         (CASE 
                              WHEN jat.jpk_section = 'SprzedazWiersz' 
                              THEN STRING_AGG(DISTINCT jpk_gtu.name, ',')
                             END)                                 AS GTU,
    ...
    def get_xml_extended(self, options):
        ...
        for line in sections.get('SprzedazWiersz', []):
            ...
            if line['data']['nrksef']:
                ...
                etree.SubElement(sale_row, etree.QName(tns, 'BFK')).text = '1'
            else:
        ...
        for line in sections.get('ZakupWiersz', []):
            ...
            if line['data']['nrksef']:
                ...
                etree.SubElement(purchase_row, etree.QName(tns, 'BFK')).text = '1'
            else:
