...
class MailMessage(models.Model):
    ...
    lead_id = fields.Many2one('crm.lead', 'Lead / Opportunity', compute='_compute_links')
    ticket_id = fields.Many2one('helpdesk.ticket', 'Helpdesk Ticket', compute='_compute_links')
    ...
    def action_open_conv_history(self, model_tech_name):
        if model_tech_name not in ('crm.lead', 'helpdesk.ticket'):
            raise oWarning('Appliable for leads and tickets only.')

        tree_view = self.env.ref('efarmer_conversation_history.mail_message_view_tree_conv_history')
        form_view = self.env.ref('efarmer_conversation_history.mail_message_view_form_conv_history')
        search_view = self.env.ref('efarmer_conversation_history.mail_message_search_form_conv_history')
