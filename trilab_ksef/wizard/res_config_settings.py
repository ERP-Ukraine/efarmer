...
class ResConfigSettings(models.TransientModel):
    ...
    x_ksef_enable_seller_contact_info = fields.Boolean(
        related='company_id.x_ksef_enable_seller_contact_info',
        readonly=False,
    )
    x_ksef_enable_buyer_contact_info = fields.Boolean(
        related='company_id.x_ksef_enable_buyer_contact_info',
        readonly=False,
    )
    x_ksef_enable_ignore_zero_amount_lines = fields.Boolean(
        related='company_id.x_ksef_enable_ignore_zero_amount_lines',
        readonly=False,
    )
    x_ksef_enable_outgoing_stock_pickings = fields.Boolean(
        related='company_id.x_ksef_enable_outgoing_stock_pickings',
        readonly=False,
    )
    x_ksef_disable_shipping_third_party = fields.Boolean(
        related='company_id.x_ksef_disable_shipping_third_party',
        readonly=False,
    )
    x_ksef_purchase_journal_id = fields.Many2one(
        related='company_id.x_ksef_purchase_journal_id',
        readonly=False,
    )
    x_ksef_fallback_purchase_journal_id = fields.Many2one(
        related='company_id.x_ksef_fallback_purchase_journal_id',
        readonly=False,
    )
    x_ksef_purchase_invoice_sync_date = fields.Datetime(
        related='company_id.x_ksef_purchase_invoice_sync_date',
        readonly=False,
    )

    x_ksef_plain_token = fields.Char(
        related='company_id.x_ksef_plain_token',
        readonly=False,
    )
    x_ksef_plain_verification_link_certificate_datas = fields.Binary(
        related='company_id.x_ksef_plain_verification_link_certificate_datas',
        readonly=False,
    )
    x_ksef_verification_link_certificate_name = fields.Char(
        related='company_id.x_ksef_verification_link_certificate_name',
        readonly=False,
    )

    x_ksef_plain_verification_link_private_key_datas = fields.Binary(
        related='company_id.x_ksef_plain_verification_link_private_key_datas',
        readonly=False,
    )
    x_ksef_verification_link_private_key_name = fields.Char(
        related='company_id.x_ksef_verification_link_private_key_name',
        readonly=False,
    )

    x_ksef_plain_verification_link_private_key_password = fields.Char(
        related='company_id.x_ksef_plain_verification_link_private_key_password',
        readonly=False,
    )
