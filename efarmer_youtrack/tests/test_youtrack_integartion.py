...
class TestYoutrackIntegration(TestYoutrackIntegrationCommon):
    ...
    def test_send_youtrack_request(self):
        ...
        with self.cr.savepoint(), patch.object(
            requests, "get", return_value=get_result
        ):
    ...
    def test_api_job_created(self):
        ...
        # enable jobs
        self.yt_integration.with_context(queue_job__no_delay=False).run_operation()

        pattern_job_func_string = (
            "youtrack.integration({},).api_get_project('{}')".format(
    ...
    def test_api_get_project(self):
        ...
        mock_request = self._create_patch_object(
            type(self.yt_integration), "_send_youtrack_request"
        )
    ...
    def test_api_get_employees(self):
        ...
        mock_create_employee = self._create_patch_object(
            type(self.env["hr.employee"]), "create"
        )
        mock_write_employee = self._create_patch_object(
            type(self.env["hr.employee"]), "write"
        )
    ...
    def test_api_get_work_items(self):
        ...
        mock_create_work_type = self._create_patch_object(
            type(self.env["youtrack.work.type"]), "create"
        )
        self.yt_integration.youtrackIntegrationApiGetWorkItems()
        created = ts_obj.search([("youtrack_id", "=", project_youtrack_id)])
    ...
    def test_api_get_task(self):
        ...
        child_task = self.env["project.task"].search(
            [("youtrack_id", "=", child_task_ext_id)]
        )
        parent_task = self.env["project.task"].search(
            [("youtrack_id", "=", parent_task_ext_id)]
        )
        ...
        self.assertTrue(
            all(
                [
                    parent_task.is_epic,
                    parent_task.asset_id,
                    parent_task.task_code,
                    parent_task.product_version_id,
                    parent_task.issue_type_id,
                    parent_task.name_pl,
                ]
            )
        )
        self.assertEqual(parent_task.allocated_hours, 550 // 60 + 550 % 60 / 60)
        ...
        mock_create_task = self._create_patch_object(
            type(self.env["project.task"]), "create"
        )
        self.yt_integration.api_get_task(child_task_ext_id)
    ...
    def test_external_object_unique(self):
        ...
        with self.assertRaises(ValidationError, msg=msg):
            self.yt_integration._get_object(
                model, [(field, "=", value)], check_unique=True
            )
