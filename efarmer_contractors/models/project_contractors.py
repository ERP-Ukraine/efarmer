...
class ProjectContractors(models.Model):
    ...
    state = fields.Selection(
        [("new", "New"), ("in_progress", "In Progress"), ("done", "Paid")],
        string="Status",
        default="new",
        tracking=True,
    )
    ...
    def generate_report(self):
        self.ensure_one()
        self.env["project.contractors.line"].search(
        ...
        analytic_lines = (
            ...
            .mapped(
                lambda line: data_dict.update(
                    {
                        line.employee_id.id: data_dict.get(
                            line.employee_id.id, self.env["account.analytic.line"]
                        )
                        + line
                    }
                )
        ...
        for contractor in self:
            ...
            for line in contractor.contractors_line_ids:
                ...
                account_asset_counterpart_id = (
                    line.employee_id.account_asset_counterpart_id
                )
                ...
                if contractor.product_id:
                    vendor_bill["invoice_line_ids"].append(
                        (
                            0,
                            0,
                            {
    ...
    def unlink(self):
        for line in self:
            if line.state in ["in_progress", "done"]:
                raise UserError(
                    _("You cannot delete a locked Project Invoice B2B Contactors.")
                )
    ...
    def write(self, vals):
        for line in self:
            if line.state in ["in_progress", "done"]:
                raise UserError(
                    _("You cannot edit a locked Project Invoice B2B Contactors.")
                )
