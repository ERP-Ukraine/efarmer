...
class PurchaseOrder(models.Model):
    ...
    department_id = fields.Many2one(
        related="user_id.default_department_id",
        string="Department",
        readonly=True,
        store=True,
    )
    ...
    def __get_default_currency(self, currency_id):
        ...
        return rate and rate[0].inverse_company_rate or 0

    @api.depends("amount_total")
    def _compute_amount_total_in_eur(self):
        for record in self:
            record.amount_total_in_eur = (
                record.amount_total * record.__get_default_currency(record.currency_id)
            )

    @api.depends("amount_untaxed")
    def _compute_untaxed_amount_in_eur(self):
        for record in self:
            record.untaxed_amount_in_eur = (
                record.amount_untaxed
                * record.__get_default_currency(record.currency_id)
            )

    @api.depends("amount_residual")
    def _compute_residual_amount_in_eur(self):
        for record in self:
            record.residual_amount_in_eur = (
    ...
    def create_po_activity(
        self, summary, user, date_deadline=None, activity_type_id=None
    ):
    ...
    def action_confirm_rfq(self):
        ...
        self.create_po_activity(
            "Confirm Demand for {}".format(self.name), user_to_notify
        )
    ...
    def action_confirm_demand(self):
        ...
        self.create_po_activity(
            "Approve Financial for {}".format(self.name), self.company_id.fin_manager_id
        )
