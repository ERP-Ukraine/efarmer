...
class TestSaleOrderWorkflow(TransactionCase):
    ...
    def test_order_line_name(self):
        ...
        """
        test_attr = self.env["product.attribute"].create({"name": "Test"})
        attr = self.env["product.attribute.value"].create(
            {"name": "Attribute", "attribute_id": test_attr.id}
        )
        ...
        sale_order_line = self._add_so_line(self.so, product, 1)
    ...
    def test_pick_priority_no_backorder(self):
        ...
        self.assertEqual(
            picking.sale_priority, "0", 'Picking Sale Priority must be "Low priority".'
        )
    ...
    def test_pick_priority_with_backorder(self):
        ...
        self.assertEqual(
            self.so.priority, "1", 'SO priority must be still "Medium priority".'
        )
