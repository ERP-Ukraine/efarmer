...
class AccountMove(models.Model):
    ...
    # Technical field
    # Task EF-252
    _current_rate_pln = fields.Boolean(
        string="Current Rate PLN",
        compute="_get_current_rate_pln",
    )
    ...
    def get_report_vat_text_line(self):
        ...
        lines_to_process = self.invoice_line_ids.filtered(
            lambda l: l.product_id.type == "service"
            or (l.product_id.type == "consu" and l.product_id.is_storable)
        )
        product_types = set(lines_to_process.mapped("product_id.type"))
        ...
        if (
            ...
            ({"consu"} == product_types or {"service", "consu"} == product_types)
            and self.commercial_partner_id.company_type == "company"
        ...
        for move in self:
            ...
            if move.invoice_date:
                invoice_date_rate_id = currency_pln.rate_ids.filtered(
                    lambda x: x.name
                    == _get_currency_rate_on_yesterday(move.invoice_date)
                    and x.company_id == move.company_id
                )
            ...
            if account_payment_ids:
                ...
                payment_rate_id = currency_pln.rate_ids.filtered(
                    lambda x: x.name == _get_currency_rate_on_yesterday(payment_id.date)
                    and x.company_id == move.company_id
                )
    ...
    def _recompute_amount(self):
        ...
        for line in self.line_ids.filtered(
            lambda line: not line.has_deposit_deducted()
        ):
            if is_invoice:
                # === Invoices ===
    ...
    def _prepare_tax_lines_data_for_totals_from_invoice(
        ...
        if not skip_deposit:
            return super()._prepare_tax_lines_data_for_totals_from_invoice(
                tax_line_id_filter, tax_ids_filter
            )

        return super()._prepare_tax_lines_data_for_totals_from_invoice(
            lambda aml, tax: not aml.has_deposit_deducted(),
            lambda aml, tax: not aml.has_deposit_deducted(),
        )
    ...
    def _get_tax_totals(
        ...
        skip_deposit = self._context.get("without_deposit")
        if not skip_deposit:
            return super()._get_tax_totals(
                partner, tax_lines_data, amount_total, amount_untaxed, currency
            )

        amount_total_skip_deposit, amount_untaxed_skip_deposit = (
            self._recompute_amount()
        )
        return super()._get_tax_totals(
            partner,
            tax_lines_data,
            amount_total_skip_deposit,
            amount_untaxed_skip_deposit,
            currency,
        )
...
class AccountMoveLine(models.Model):
    ...
    def has_deposit_deducted(self):
        ...
        if self.product_id.id == int(deposit_product_id):
            ...
            if (
                float_compare(self.price_subtotal, 0.0, precision_rounding=precision)
                == -1
            ):
                return True
        return False
