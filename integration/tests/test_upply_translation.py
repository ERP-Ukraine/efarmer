...
class TestTranslations(OdooIntegrationInit):
    ...
    def setUp(self):
        ...
        # install `nl`
        wizard_vals = dict(lang=self.lang_nl.code)
        wizard = self.env['base.language.install'].create(wizard_vals)
        wizard.lang_install()

        # install `en`
        wizard_vals = dict(lang=self.lang_en.code)
        wizard = self.env['base.language.install'].create(wizard_vals)
        wizard.lang_install()

        # install `de`
        wizard_vals = dict(lang=self.lang_de.code)
        wizard = self.env['base.language.install'].create(wizard_vals)
        wizard.lang_install()

        # install `it`
        wizard_vals = dict(lang=self.lang_it.code)
        wizard = self.env['base.language.install'].create(wizard_vals)
        wizard.lang_install()

        # install `pl`
        wizard_vals = dict(lang=self.lang_pl.code)
        wizard = self.env['base.language.install'].create(wizard_vals)
        wizard.lang_install()
    ...
    def test_context_language_no_matter(self):
        ...
        integration._patch_method('get_shop_lang_code', _get_default_lang_patch)
        ...
        integration._revert_method('get_shop_lang_code')
    ...
    def test_apply_translation_external_lang_not_eq_erp_language(self):
        ...
        res_lang._patch_method('from_external', _from_external_patch)

        integration._patch_method('get_adapter_lang_code', _get_adapter_lang_code_patch)
        integration._patch_method('get_shop_lang_code', _get_default_lang_patch)
        ...
        res_lang._revert_method('from_external')
        integration._revert_method('get_adapter_lang_code')
        integration._revert_method('get_shop_lang_code')
