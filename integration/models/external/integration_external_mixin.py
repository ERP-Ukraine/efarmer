...
from ...exceptions import NoExternal
...
class IntegrationExternalMixin(models.AbstractModel):
        ...
        if reference:
            ...
            if len(odoo_record) > 1:
                raise ValidationError(
                    _('Multiple Odoo records found with the same internal reference: %s --> %s')
                    % (str(search_domain), odoo_record)
                )
    ...
    def get_external_by_code(self, integration, code, raise_error=True):
        ...
        if raise_error:
            if not external:
                raise NoExternal(
                    _('Cannot find external record'), self._name, code, integration
                )
    ...
    def _fix_unmapped_element_values(self, integration, element):
        ...
        if element not in ('attribute', 'feature'):
            raise UserError(
                _('Invalid "element" value: it should be either "attribute" or "feature".')
            )
