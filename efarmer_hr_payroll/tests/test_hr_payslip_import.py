...
class TestHrPayslipImportWizard(TransactionCase):
    ...
    def test_get_existing_data(self):
        ...
        existing_employee = self.wizard._get_existing_data(
            "hr.employee", employee_import_data, "Employee"
        )
    ...
    def test_create_payslip(self):
        payslip_vals = [
            {
                self.employee_1: [
                    (self.input_type_1.id, 0.0),
                    (self.input_type_2.id, 10.0),
                ]
            },
            {
                self.employee_2: [
                    (self.input_type_1.id, 20.0),
                    (self.input_type_2.id, 30.0),
                ]
            },
        ]
        ...
        lines_expected_vals = [
            (line.input_type_id.id, line.amount) for line in payslip_2_input_lines
        ]
        self.assertEqual(
