...
class ProjectCapitalization(models.Model):
    ...
    def generate_report(self):
        self.ensure_one()
        self.env["project.capitalization.line"].search(
        ...
        for task_product_id in task_product_ids:
            lines_by_task_product_id = analytic_lines.filtered(
                lambda line: line.task_product_id == task_product_id
            )
            account_asset_counterpart_ids = set(
                lines_by_task_product_id.mapped("account_asset_counterpart_id")
            )
            for account_asset_counterpart_id in account_asset_counterpart_ids:
                lines_by_account_asset_counterpart_id = (
                    lines_by_task_product_id.filtered(
                        lambda line: line.account_asset_counterpart_id
                        == account_asset_counterpart_id
                    )
                )
                hours_spent = sum(
                    lines_by_account_asset_counterpart_id.mapped("unit_amount")
                )
            ...
            lines_without_account_asset_counterpart_id = (
                lines_by_task_product_id.filtered(
            ...
            if lines_without_account_asset_counterpart_id:
                hours_spent = sum(
                    lines_without_account_asset_counterpart_id.mapped("unit_amount")
                )
                amount = sum(
                    lines_without_account_asset_counterpart_id.mapped("amount")
                )
                capitalized_lines.append(
                    (
                        0,
    ...
    def capitalization(self):
        ...
        for line in lines:
            original_value = line.currency_id._convert(
                line.amount, line.asset_id.currency_id, self.company_id, date_today
            )
            old_values = {
            ...
            current_asset_book = (
                line.asset_id.value_residual + line.asset_id.salvage_value
            )
            increase = original_value - current_asset_book
            new_residual = min(
                current_asset_book - min(original_value, line.asset_id.salvage_value),
                0.0,
            )
            ...
            if (
                line.asset_id.currency_id.round(residual_increase + salvage_increase)
                > 0
            ):
                        ...
                        "line_ids": [
                            (
                                ...
                                0,
                                {
                                    ...
                                    "debit": line.asset_id.currency_id._convert(
                            ...
                            (
                                ...
                                0,
                                {
                                    ...
                                    "debit": 0,
                                    "credit": line.asset_id.currency_id._convert(
                                        residual_increase + salvage_increase,
                                        self.company_id.currency_id,
                                        self.company_id,
                                        date_today,
                ...
                asset_increase = line.env["account.asset"].create(
                    {
                        ...
                        "prorata_computation_type": line.asset_id.prorata_computation_type,
                        ...
                        "original_move_line_ids": [
                            (
                                ...
                                move.line_ids.filtered(
                                    ...
                                    == line.asset_id.account_asset_id
                                ).ids,
                            )
                        ],
                    }
                ...
                subject = _(
                    "A gross increase has been created"
            ...
            if increase < 0:
                ...
                move = (
                    line.env["account.move"]
                    .create(
                        line.env["account.move"]._prepare_move_for_asset_depreciation(
                            {
                                "amount": -increase,
                                "asset_id": line.asset_id,
                                "move_ref": _(
                                    "Value decrease for: %(asset)s",
                                    asset=line.asset_id.name,
                                ),
                                "date": self.capitalization_date,
            ...
            changes, tracking_value_ids = line.asset_id._mail_track(
                tracked_fields, old_values
            )
            if changes:
                line.asset_id.message_post(
                    body=_("Depreciation board modified") + "<br>" + self.name,
    ...
    def line_capitalize(self):
        for capitalization in self:
            ...
            capitalization.env["account.analytic.line"].search(domain).write(
                {"is_capitalized": True}
            )
    ...
    def open_analytic_lines(self):
        ...
        action["context"] = {
            "group_by": ["task_product_id", "account_asset_counterpart_id"]
        }
        return action
        ...
        for vals in vals_list:
            if vals.get("name", _("New")) == _("New"):
                vals["name"] = self.env["ir.sequence"].next_by_code(
                    "project.capitalization.sequence"
                ) or _("New")
        return super(ProjectCapitalization, self).create(vals_list)
