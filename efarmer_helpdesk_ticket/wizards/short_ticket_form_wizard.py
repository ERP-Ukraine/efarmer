...
class ShortTicketFormWizard(models.TransientModel):
    ...
    lot_id = fields.Many2one(
        ...
        string="Lot/Serial",
        domain=lambda self: self.get_lot_domain(),
    )
    ...
    def get_lot_domain(self):
        if self._context.get("choose_product_by") == "serial":
            lot_ids = (
                self.env["stock.lot"]
                .search([])
                .filtered(lambda l: l.product_id.tracking == "serial")
            )
            ...
            lot_ids = (
                self.env["stock.lot"]
                .search([])
                .filtered(lambda l: l.product_id.tracking == "lot")
            )
            return [("id", "in", lot_ids.ids)]
    ...
    def _onchange_choose_product_by(self):
        ...
        if self.choose_product_by == "serial":
            ...
            lot_domain.append(("id", "in", lot_ids.ids))
        else:
            lot_ids = (
                self.env["stock.lot"]
                .search([])
            ...
            product_domain.append(("id", "in", product_ids.ids))
        return {
            "domain": {
    ...
    def _onchange_partner_id(self):
        ...
        if self.choose_product_by == "other" and self.partner_id:
            ...
            sale_domain.append(("id", "in", sale_ids.ids))
            return {
