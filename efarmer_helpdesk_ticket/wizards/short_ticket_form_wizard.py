...
class ShortTicketFormWizard(models.TransientModel):
    ...
    def _onchange_lot_id(self):
        if not self.lot_id:
            ...
            self.sale_id = self.env['stock.move.line'].search([('lot_id', '=', self.lot_id.id)]).mapped('move_id').mapped('picking_id').mapped('sale_id')[:1]
            ...
            product_domain = []
            product_ids = self.env['product.product'].search([('tracking', 'in', ['lot', 'no_tracking'])])
    ...
    def onchange_sale_domain(self):
        sale_domain = []
        lot_domain = []
        if self.choose_product_by == 'other' and not self.product_id:
            self.sale_id = False
            return {
        ...
        if self.choose_product_by == 'other' and self.product_id:
            ...
            sale_ids = self.env['stock.move.line'].search(search_domain).mapped('move_id').mapped('picking_id').mapped('sale_id')
            sale_domain.append(
                ('id', 'in', sale_ids.ids))
            return {
                'domain': {'sale_id': sale_domain,
